...
description: Download OPERA L2 RTC-S1 VV Cloud-Optimized GeoTIFFs from ASF via earthaccess. Use when fetching Sentinel-1 RTC backscatter, estimating download size, or when ASF datapool returns 401 Unauthorized.
---
...
5. Authenticate, then `earthaccess.download(urls, provider="ASF")`.
   See **Auth**. Do not use `earthaccess.open()` for ASF datapool.
...
- ASF **datapool download** needs `~/.netrc`:
  ...
  `earthaccess.download(urls, provider="ASF")`
